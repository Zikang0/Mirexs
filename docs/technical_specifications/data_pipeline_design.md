...
*   **核心组件**：
    ...
    *   **数据采集器 (Data Collectors)**：
        ...
        *   **外部知识采集**：通过 `real_time_knowledge.py` 模块，定期或实时抓取 RSS Feeds、新闻 API、学术论文等外部信息源。
...
*   **核心组件**：
    ...
    *   **知识图谱构建**：
        ...
        *   **关系抽取 (RE)**：识别实体之间的语义关系。
        *   **事件抽取 (EE)**：识别并结构化描述事件。
        *   **消歧与融合**：解决实体指代模糊和实体合并问题，将新信息整合到 `graph_db/knowledge_graph.py` 中。
...
*   **核心组件**：
    ...
    *   **向量数据库 (Vector Database)**：`vector_db/` 存储所有实体、文档块、用户偏好等的向量嵌入，支持高效的相似度搜索。主要采用 Chroma，备用 FAISS。
    *   **图数据库 (Graph Database)**：`graph_db/` 存储 Mirexs 的知识图谱，包括实体、关系及其属性。主要采用 Neo4j。
    *   **关系型数据库 (Relational Database)**：`relational/` 存储用户配置、系统元数据、任务状态等结构化数据。可选用 PostgreSQL 或 MySQL。
    *   **时序数据库 (Time-Series Database)**：`time_series/` 存储系统日志、性能指标等时间序列数据。可选用 Prometheus 或 InfluxDB。
